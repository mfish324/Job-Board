...
- **Salary extraction from description text** (2026-08-13) — `jobs/salary_extract.py`: conservative regex extractor that parses pay-transparency ranges out of description text (annualizes hourly x2080 / weekly x52 / monthly x12; rejects bonus/401k/revenue mentions and non-USD; sentence-scoped context windows; unescapes double-encoded HTML). Wired into `sync_genzjobs._sync_listing` as a fallback when the feed provides no structured salary, plus a `extract_salaries` management command for backfill (`--dry-run`, `--limit`, `--all-statuses`). Motivation: direct-ATS feeds (Anthropic, OpenAI, Salesforce...) carry salary only in description text, so the HAS specificity signal (`salary_min/salary_max` check) was blind to the most salary-transparent employers, and the homepage employer table briefly showed them at 0% salary listed (column pulled in the honesty patch; reinstate after backfill). Backfill must run on prod (Render shell: `python manage.py extract_salaries --dry-run` then without the flag), then scores update at the next daily rescore.
- **Filter-first homepage v2** (2026-08-13) — landing page redesigned around the HAS filter identity instead of the aggregator template. Hero leads with tracked-vs-published counts ("We're tracking X listings. Only Y made the cut."); anonymized "Rejected" ticker rotates real unpublished listings (job category + title only — company names deliberately withheld since a low score is a probability estimate, not an accusation); "What changed since yesterday" delta block (new 24h / closed 24h via `date_removed` / reposts caught / companies ramping via `CompanyHiringProfile.net_job_movement_30d` / gone quiet); "Beat the crowd" freshness tiers linking into `/jobs/` with real filter params (`date_posted`, `sort=newest`); fresh cards show salary (or an honest "No salary listed"), "Discovered Xh ago", `has_indicator`, and per-card "why it's here" chips; "Employers ranked by behavior" table (avg HAS, new this week, salary %, live count — one grouped aggregate over the published set) REPLACES the homepage Featured Employers career-portal cards (directory still exists at `/directory/` and is linked from the table header); category pills demoted to secondary nav near the footer. `home()` rewritten: cache key bumped to `home_page_data_v2`, same 30-min cache pattern as v1 (all heavy queries stay inside the cache rebuild; raw model instances cached, wrapped per-request). All page-specific CSS lives in home.html's `extra_css` block (`v2-` prefixed classes); base.html untouched.
