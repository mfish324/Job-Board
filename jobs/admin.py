...
from .models import (Job, UserProfile, JobApplication, PhoneVerification, EmailVerification, SavedJob,
                     ...
                     DailyGhostReport, SocialContentDraft, DailyScoreSnapshot)
...
@admin.register(DailyGhostReport)
class DailyGhostReportAdmin(admin.ModelAdmin):
    """Read-only — these rows are generated by generate_daily_report, not hand-edited."""
    list_display = ('date', 'industry_category', 'total_listings', 'ghost_rate',
                    'ghost_rate_delta', 'avg_has', 'median_has')
    list_filter = ('industry_category', 'date')
    date_hierarchy = 'date'
    ordering = ('-date', 'industry_category')

    @admin.display(description='Δ Ghost Rate')
    def ghost_rate_delta(self, obj):
        if obj.previous_ghost_rate is None:
            return '—'
        delta = obj.ghost_rate - obj.previous_ghost_rate
    ...
    def has_change_permission(self, request, obj=None):
        return False  # read-only view; rows are generated

    def has_delete_permission(self, request, obj=None):
        return True  # allow cleanup of bad/test runs


@admin.register(SocialContentDraft)
class SocialContentDraftAdmin(admin.ModelAdmin):
    list_display = ('date', 'platform', 'status', 'draft_preview')
    list_filter = ('status', 'platform', 'date')
    list_editable = ('status',)
    date_hierarchy = 'date'
    ordering = ('-date', '-created_at')
    readonly_fields = ('source_metrics', 'created_at', 'updated_at')
    fields = ('date', 'platform', 'status', 'draft_text', 'source_metrics',
              'created_at', 'updated_at')

    @admin.display(description='Draft')
    def draft_preview(self, obj):
        return (obj.draft_text or '')[:100]


@admin.register(DailyScoreSnapshot)
class DailyScoreSnapshotAdmin(admin.ModelAdmin):
    """Per-listing daily score history backing the day-over-day deltas."""
    list_display = ('date', 'listing_id', 'company_name', 'industry_category', 'total_score')
    list_filter = ('date', 'industry_category')
    search_fields = ('company_name', 'listing_id')
    date_hierarchy = 'date'
