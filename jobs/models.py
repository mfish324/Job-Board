...
# =============================================================================
# DAILY GHOST JOB ANALYSIS PIPELINE
# =============================================================================

class DailyScoreSnapshot(models.Model):
    """
    Per-listing HAS score captured once per day.

    The daily rescore cron (`score_listings --force`) overwrites each
    HiringActivityScore in place, so there is no per-listing score history.
    This table snapshots scores each day so `generate_daily_report` can compute
    day-over-day deltas: threshold crossings (a listing that fell below 65) and
    per-company HAS movers.

    Stores `listing_id` as a plain integer (not a FK) so a snapshot survives the
    underlying ScrapedJobListing being deleted/expired — we only need the score
    history, not a live relation. Pruned to ~35 days by generate_daily_report.
    """
    ...
    industry_category = models.CharField(max_length=50, blank=True, null=True, db_index=True)
    total_score = models.PositiveSmallIntegerField()
    ...
    class Meta:
        constraints = [
            models.UniqueConstraint(
                fields=['date', 'listing_id'],
                name='uniq_snapshot_date_listing',
            ),
        ]
        indexes = [
            models.Index(fields=['date', 'industry_category']),
        ]
        verbose_name = 'Daily Score Snapshot'
        verbose_name_plural = 'Daily Score Snapshots'

    def __str__(self):
        return f"{self.date} listing#{self.listing_id}: {self.total_score}"


class DailyGhostReport(models.Model):
    """
    One row per industry per day — aggregated HAS metrics behind the recurring
    "Hiring Activity Report" content series.

    Generated by `generate_daily_report` from existing stored HAS scores; this
    model never recomputes scores. Idempotent on (date, industry_category).
    """
    date = models.DateField(db_index=True)
    # Matches ScrapedJobListing.industry_category (genzjobs IndustryCategory
    # enum, e.g. 'TECHNOLOGY'). Stored verbatim; nullable industries are excluded
    # from reporting, so this is always populated here.
    industry_category = models.CharField(max_length=50)

    # Volume + threshold split
    total_listings = models.PositiveIntegerField(default=0)
    above_threshold_count = models.PositiveIntegerField(
    ...
    # Headline rates (percentages 0.00-100.00)
    ghost_rate = models.DecimalField(
        max_digits=5, decimal_places=2, default=0,
        help_text='% of listings scoring below threshold'
    )
    avg_has = models.DecimalField(max_digits=5, decimal_places=2, default=0)
    median_has = models.DecimalField(max_digits=5, decimal_places=2, default=0)
    salary_transparency_rate = models.DecimalField(
        max_digits=5, decimal_places=2, default=0,
        help_text='% of listings with salary_min or salary_max present'
    )
    repost_rate = models.DecimalField(
        max_digits=5, decimal_places=2, default=0,
        help_text='% of listings flagged with 3+ reposts (repost_count >= 3)'
    )
    evergreen_rate = models.DecimalField(
        max_digits=5, decimal_places=2, default=0,
        help_text='% open 90+ days with no updates (repost_count == 0) — '
                  'matches the HAS evergreen_penalty signal'
    )

    # Age distribution: counts by days_since_first_seen bucket
    staleness_buckets = models.JSONField(
        default=dict,
        help_text='{"0_14d": N, "15_30d": N, "31_60d": N, "61_90d": N, "90d_plus": N}'
    )

    # New listings arriving today
    new_listings_today = models.PositiveIntegerField(
        default=0, help_text='Listings first seen in the last 24 hours'
    )
    new_listings_avg_has = models.DecimalField(
        max_digits=5, decimal_places=2, null=True, blank=True,
        help_text="Average HAS of today's new listings (null if none)"
    )

    # Day-over-day movement (from DailyScoreSnapshot; 0/empty on first run)
    threshold_crossings_down = models.PositiveIntegerField(
        default=0,
        help_text='Listings that were >= 65 yesterday but < 65 today'
    )
    top_movers = models.JSONField(
        default=dict,
        help_text='{"up": [{"company", "delta", "current_avg"}], "down": [...]} '
                  '— top 5 each direction by avg-HAS change vs prior day'
    )

    # Trend reference
    previous_ghost_rate = models.DecimalField(
        max_digits=5, decimal_places=2, null=True, blank=True,
    ...
    def __str__(self):
        return f"{self.date} {self.industry_category}: {self.ghost_rate}% ghost"


class SocialContentDraft(models.Model):
    """
    LinkedIn (or other platform) post drafts generated from a DailyGhostReport.
    Reviewed and approved manually in admin before posting — no auto-posting.
    """
    STATUS_CHOICES = [
        ('draft', 'Draft'),
        ('approved', 'Approved'),
    ...
    date = models.DateField(db_index=True)
    platform = models.CharField(max_length=30, default='linkedin')
    draft_text = models.TextField()
    source_metrics = models.JSONField(
        default=dict,
        help_text='Snapshot of the data points that informed this draft '
                  '(for traceability back to the numbers)'
    )
    ...
    def __str__(self):
        return f"{self.date} {self.platform} ({self.status})"
